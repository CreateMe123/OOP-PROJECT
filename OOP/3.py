...
def compute():
    try:
        ...
        if not name:
            ...
            errors.append("Invalid Name: must not contain digits.")
        if position not in pay_data:
            errors.append("Invalid Position selected.")
        if pay_grade not in ("A", "B"):
            errors.append("Please select a Pay Grade (A or B).")
        if ot_hours < 0 or ot_hours > 24:
            errors.append("OT Hours must be between 0 and 24.")
        if errors:
            raise ValueError("\n".join(errors))
